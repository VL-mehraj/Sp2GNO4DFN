...
from models.discrete_f_n.Sp2GNO import GraphFNO
...
def create_log_folder():
    ...
    log_folder = f"./log/logs_{timestamp}"
    os.makedirs(log_folder)
    path = f'./{log_folder}/{dataset_name}_Res/'
    path_model =  path+'model/'
    path_image_test61 = path +'image/'
...
PATH = '..'
TRAIN_PATH = PATH + "/data/dfn/dfn_4frac_prob.mat"
TEST_PATH =  PATH + "/data/dfn/dfn_4frac_prob.mat"
...
class Dataset:
    def __init__(self, train_path, s, T_in , T_out, device, gtype= 'knn',  dist_weighted_laplacian = False):
        ...
        print('after U to device')
        

    def read_data(self, TRAIN_PATH):
        reader = MatReader(TRAIN_PATH)
        self.train_a = reader.read_field('coeff')[:ntrain, :, :, :T_in].flatten(1,2)
        self.train_u = reader.read_field('sol')[:ntrain, :, :, T_in:T_in + T_out].flatten(1,2)

        self.test_a = reader.read_field('coeff')[-ntest:, :, :, :T_in].flatten(1,2)
        self.test_u = reader.read_field('sol')[-ntest:, :, :, T_in:T_in + T_out].flatten(1,2)

        print("train_a.shape", self.train_a.shape)
        print("test_a.shape", self.test_a.shape)
        print("train_u.shape", self.train_u.shape)
        print("test_u.shape", self.test_u.shape)
    
        # Reshape: We want to keep the nodes and features together, so we reshape accordingly
        self.train_a = self.train_a.reshape(ntrain, 3890 * 6, T_in)  # Use 3890 nodes and 6 features
        self.test_a = self.test_a.reshape(ntest, 3890 * 6, T_in)
...
for ep in range(epochs):
    ...
    if ep%100==0:
        ...
        plt.savefig(path_image_test61 + f"dfn_test_loss_{test_l2_full/ntest}.png")
        plt.close()
